...
const HELLO_KITTY_STAGES = [
  { min: 0, max: 499, label: "Bebe 🍼", size: 48, emoji: "🐱" },
  ...
  { min: 10000, max: Infinity, label: "Hello Kitty legendaire 🌈", size: 120, emoji: "🦄" },
];
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (mode === "signup") {
        ...
        setSuccess("Compte cree ! Verifie tes emails pour confirmer 🌸");
      } else {
    ...
    <div style={{ minHeight: "100vh", background: `linear-gradient(135deg, ${PASTEL.rosePale} 0%, ${PASTEL.cream} 50%, ${PASTEL.lilac} 100%)`, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Lato', sans-serif", padding: 24 }}>
      ...
      <div style={{ textAlign: "center", animation: "fadeIn 0.6s ease", width: "100%", maxWidth: 380 }}>
        ...
        <div style={{ background: PASTEL.white, borderRadius: 24, padding: 28, boxShadow: "0 20px 60px rgba(244,130,154,0.2)" }}>
          ...
          {mode === "signup" && <Input label="Ton prenom 🌸" value={pseudo} onChange={setPseudo} placeholder="ex: Sophie" />}
          ...
          <PinkButton onClick={handleSubmit} disabled={loading}>{loading ? "Chargement..." : mode === "login" ? "Se connecter ✨" : "Creer mon compte 🌸"}</PinkButton>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div style={{ minHeight: "100vh", background: `linear-gradient(135deg, ${PASTEL.rosePale}, ${PASTEL.cream})`, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Lato', sans-serif", padding: 24 }}>
      ...
      <div style={{ background: PASTEL.white, borderRadius: 24, padding: 28, maxWidth: 520, boxShadow: "0 20px 60px rgba(244,130,154,0.2)" }}>
        ...
        <h2 style={{ fontFamily: "'Playfair Display', serif", color: PASTEL.roseDeep, textAlign: "center", marginBottom: 12, fontSize: 20 }}>Une derniere etape !</h2>
        ...
        <div style={{ marginTop: 16, textAlign: "center" }}>
          <PinkButton onClick={() => window.location.reload()}>J'ai termine, recharger 🌸</PinkButton>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
        {[
          { label: "Revenus nets", value: `${stats.revenue.toFixed(2)}€`, color: "#5BA85A", emoji: "💚", bg: "#F0FFF4" },
          { label: "Depenses", value: `${stats.expenses.toFixed(2)}€`, color: "#E85A5A", emoji: "💸", bg: "#FFF5F5" },
          { label: "Solde net", value: `${net.toFixed(2)}€`, color: net >= 0 ? "#5BA85A" : "#E85A5A", emoji: "🏦", bg: PASTEL.cream },
          { label: "Stock en attente", value: `${stats.stock} articles`, color: PASTEL.roseDark, emoji: "📦", bg: PASTEL.rosePale }
        ].map(c => (
  ...
  return (
    <div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12, marginBottom: 20 }}>
        {[
          { label: "Benefice total", value: `${totalRevenue.toFixed(2)}€`, emoji: "💰" },
          { label: "Ventes", value: sales.length, emoji: "🛍️" },
          { label: "Meilleure vente", value: topItem ? `${topItem.profit.toFixed(0)}€` : "—", emoji: "⭐" }
        ].map(c => (
      ...
      {mostSold && <div style={{ background: `linear-gradient(135deg, ${PASTEL.rosePale}, ${PASTEL.cream})`, borderRadius: 16, padding: "12px 16px", marginBottom: 16, border: `1px solid ${PASTEL.rose}` }}>
        ...
        <span style={{ fontSize: 12, color: PASTEL.textLight }}> x {mostSold[1]}</span>
      </div>}
        ...
        {sales.map(s => (
          <div key={s.id} style={{ background: PASTEL.white, borderRadius: 16, padding: "14px 16px", border: `1px solid ${PASTEL.lilac}`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <div>
              ...
              <div style={{ fontSize: 11, color: PASTEL.textLight, marginTop: 2 }}>Achete {s.buy_price}€ → Vendu {s.sold_price}€ · {s.date}</div>
            ...
            <div style={{ textAlign: "right" }}>
              ...
              <div style={{ fontSize: 10, color: PASTEL.textLight }}>benefice</div>
            </div>
          </div>
        ))}
      </div>
      {modal && (
        <Modal title="🛍️ Nouvelle vente" onClose={() => setModal(false)}>
          ...
          <Input label="Nombre de personnes" value={form.partners} onChange={v => setForm(p=>({...p,partners:v}))} type="number" placeholder={`${partnerCount} (defaut)`} />
          {form.soldPrice && form.buyPrice && (
            <div style={{ background: PASTEL.rosePale, borderRadius: 12, padding: "10px 14px", marginBottom: 14, fontSize: 13 }}>
              💸 Benefice : <strong style={{ color: PASTEL.roseDeep }}>{(parseFloat(form.soldPrice||0)-parseFloat(form.buyPrice||0)).toFixed(2)}€</strong>
    ...
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
        <div>
          ...
          <div style={{ fontSize: 11, color: PASTEL.textLight }}>{stock.length} article(s) a vendre</div>
        </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        {stock.map(s => (
          <div key={s.id} style={{ background: PASTEL.white, borderRadius: 16, padding: "14px 16px", border: `1px solid ${PASTEL.lilac}`, display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
            <div>
              <div style={{ fontFamily: "'Playfair Display', serif", fontWeight: 700, color: PASTEL.text, fontSize: 15 }}>{s.article}</div>
              <div style={{ fontSize: 11, color: PASTEL.textLight, marginTop: 2 }}>Achete {s.buy_price}€ · Objectif {s.target_price}€</div>
    ...
    <div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 16 }}>
        <div style={{ background: PASTEL.cream, borderRadius: 16, padding: "14px 12px", textAlign: "center", border: `1px solid ${PASTEL.lilac}` }}>
          ...
          <div style={{ fontSize: 11, color: PASTEL.textLight }}>Total depenses</div>
        </div>
        <div style={{ background: PASTEL.cream, borderRadius: 16, padding: "14px 12px", border: `1px solid ${PASTEL.lilac}` }}>
          ...
          {byCat.length === 0 && <div style={{ fontSize: 12, color: PASTEL.textLight, textAlign: "center", paddingTop: 8 }}>Aucune categorie</div>}
        </div>
      </div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
        <h3 style={{ margin: 0, fontFamily: "'Playfair Display', serif", color: PASTEL.roseDeep, fontSize: 16 }}>Mes depenses</h3>
        <PinkButton onClick={() => setModal(true)} small>+ Ajouter</PinkButton>
      </div>
      {loading && <div style={{ textAlign: "center", padding: 32, color: PASTEL.textLight }}>Chargement... 🌸</div>}
      {!loading && expenses.length === 0 && <div style={{ textAlign: "center", padding: 32, color: PASTEL.textLight, fontSize: 13 }}>🎀 Aucune depense enregistree !</div>}
      ...
      {modal && (
        <Modal title="💸 Nouvelle depense" onClose={() => setModal(false)}>
          ...
          <div style={{ marginBottom: 14 }}>
            <label style={{ fontSize: 12, color: PASTEL.textLight, fontFamily: "'Playfair Display', serif", fontWeight: 600, display: "block", marginBottom: 4 }}>Categorie</label>
...
function ProjectsSection({ userId, triggerSticker }) {
  ...
  return (
    <div>
      ...
      {loading && <div style={{ textAlign: "center", padding: 32, color: PASTEL.textLight }}>Chargement... 🌸</div>}
      {!loading && projects.length === 0 && <div style={{ textAlign: "center", padding: 32, color: PASTEL.textLight, fontSize: 13 }}>🌟 Aucun projet. Reve grand !</div>}
      ...
      {modal && (
        <Modal title="🌟 Nouveau projet" onClose={() => setModal(false)}>
          ...
          <Input label="Deja economise (€)" value={form.saved} onChange={v => setForm(p=>({...p,saved:v}))} type="number" placeholder="0" />
          <div style={{ marginBottom: 14 }}>
          ...
          <PinkButton onClick={handleAdd}>Creer le projet 💫</PinkButton>
        </Modal>
      )}
    </div>
  );
...
export default function Bookiara() {
  ...
  const pseudo = session?.user?.user_metadata?.pseudo || session?.user?.email?.split("@")[0] || "cherie";
  const sections = [
    { id: "dashboard", label: "🏠 Tableau de bord" },
    { id: "sales", label: "🛍️ Mes ventes" },
    { id: "stock", label: "📦 Mon stock" },
    { id: "expenses", label: "💸 Mes depenses" },
        ...
        <div style={{ position: "fixed", inset: 0, zIndex: 200 }} onClick={() => setMenuOpen(false)}>
          <div style={{ position: "absolute", right: 0, top: 0, bottom: 0, width: 260, background: PASTEL.white, boxShadow: "-8px 0 40px rgba(244,130,154,0.2)", animation: "slideIn 0.25s ease", padding: "24px 20px", overflowY: "auto" }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ marginTop: 16, paddingTop: 16, borderTop: `1px solid ${PASTEL.lilac}` }}>
              <div style={{ fontSize: 12, color: PASTEL.textLight, marginBottom: 8 }}>Personnes par vente (defaut)</div>
              ...
              <button onClick={() => supabase.auth.signOut()} style={{ background: PASTEL.rosePale, border: "none", borderRadius: 12, padding: "10px 16px", cursor: "pointer", color: PASTEL.roseDeep, fontFamily: "'Playfair Display', serif", fontWeight: 600, fontSize: 13, width: "100%" }}>Se deconnecter 👋</button>
            </div>
          </div>
        </div>
      )}
